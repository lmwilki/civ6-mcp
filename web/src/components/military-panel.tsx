    ...
    >
      ...
      <div className="mb-2 flex gap-4 text-xs">
        <span className="text-marble-600">
          Total: <span className="font-mono tabular-nums text-marble-800"><AnimatedNumber value={agent.units_total} decimals={0} /></span>
        </span>
        <span className="text-marble-600">
          Combat: <span className="font-mono tabular-nums text-marble-800">{agent.units_military}</span>
        </span>
        <span className="text-marble-600">
          Civilian: <span className="font-mono tabular-nums text-marble-800">{agent.units_civilian}</span>
        </span>
        <span className="text-marble-600">
          Support: <span className="font-mono tabular-nums text-marble-800">{agent.units_support}</span>
        </span>
      </div>
            ...
            .map(([type, count]) => (
              <div key={type} className="rounded-sm bg-marble-100 px-2 py-0.5">
                <span className="font-mono text-xs text-marble-700">
                  {type.replace(/_/g, " ")} <span className="text-marble-500">x{count}</span>
                </span>
              </div>
            ))}
        </div>
